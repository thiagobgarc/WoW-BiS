...
import { hasBlizzardCredentials, MOCK_EQUIPMENT, MOCK_MEDIA, MOCK_PROFILE, MOCK_STATISTICS } from './mock';
import {
...
export async function getRealmIndex(region: string): Promise<RealmIndex> {
  return cached(`realm-index:${region}`, TTL_REALM_INDEX_SECONDS, async () => {
    const raw = await blizzardGet<unknown>('/data/wow/realm/index', { namespace: 'static', region });
